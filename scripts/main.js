...
//The day colesdle was launched. Used to find game number each day
const colesdleStartDate = new Date("07/27/2024");
...
function fetchGameData(gameNumber) {
  fetch("./games.json")
    ...
    .then((json) => {
      ...
      productPrice = Number(productPrice.slice(1, productPrice.length)).toFixed(2);;
...
function updateGuessStat() {
  ...
  if (gameState.hasWon) {
    ...
    guessStats.innerHTML += `<center>The price was $${parseFloat(productPrice).toFixed(2)}</center>`;
    return;
  }
...
function copyStats() {
  let output = `colesdle #${gameNumber}`;
  if (!gameState.hasWon) {
    output += ` X/6\n`;
  } else {
...
function switchState(event) {
  ...
  if (overlayElem.style.display === "flex") {
    title.innerHTML = `coles<span class="coles-blue">dle</span>`;
  ...
  function renderInfo() {
    title.innerHTML = `HOW TO <span class="coles-blue">PLAY</span>`;
    if (!title.classList.contains("info-title")) {
  ...
  function renderStats() {
    title.innerHTML = `GAME <span class="coles-blue">STATS</span>`;
...
/*
  Finds current game number based off of colesdle start date
*/

function getGameNumber() {
  const currDate = new Date();
  let timeDifference = currDate.getTime() - colesdleStartDate.getTime();
